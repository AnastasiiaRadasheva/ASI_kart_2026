...
public class CarController2 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (Input.GetKey(KeyCode.UpArrow)) targetVertical = 0.5f;
        if (Input.GetKey(KeyCode.DownArrow)) targetVertical = -0.5f;

        if (Input.GetKey(KeyCode.RightArrow)) targetHorizontal = 0.5f;
        if (Input.GetKey(KeyCode.LeftArrow)) targetHorizontal = -0.5f;
